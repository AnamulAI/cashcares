...
export function SummaryCards() {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
      ...
      <StatCard icon={<PieChart className="h-5 w-5 text-warning" />} iconBg="bg-warning/10" label={t("dashboard.budgetUsage")} value={monthExpense > 0 ? `${Math.round((monthExpense / (monthIncome || 1)) * 100)}%` : "0%"} trendType="neutral" />
    </div>
  );
}
