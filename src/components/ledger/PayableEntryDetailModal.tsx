...
export function PayableEntryDetailModal({ entry, open, onOpenChange, formatAmount, formatDate }: Props) {
  const { data: history = [], isLoading } = usePaymentHistory(open ? entry?.id : undefined);
  if (!entry) return null;

  const balance = Number(entry.amount) - Number(entry.paid_amount);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>{entry.description || "Payable entry"}</DialogDescription>
        </DialogHeader>
        ...
        <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
          ...
          <div><span className="text-muted-foreground">Paid:</span> <span className="font-semibold text-positive">{formatAmount(Number(entry.paid_amount))}</span></div>
        ...
        <div className="mt-4">
          ...
          {isLoading ? (
            ...
            <EmptyState title="No payment history yet" className="py-6" />
          ) : (
          ...
          {history.length > 0 && (
            <p className="text-xs text-muted-foreground mt-1.5">{history.length} payment{history.length > 1 ? "s" : ""} recorded</p>
          )}
        </div>
      </DialogContent>
    </Dialog>
